...
function parseMedicalFAQ(content) {
  ...
  for (let i = 1; i < questionBlocks.length; i++) {
    ...
    } else if (questionLower.includes('pharmacy') || questionLower.includes('medication')) {
      subcategory = 'Pharmacy Services';
    } else if (questionLower.includes('first aid') || questionLower.includes('first-aid')) {
      subcategory = 'First Aid';
    } else if (questionLower.includes('emergency') || questionLower.includes('urgent')) {
      subcategory = 'Emergency Services';
    } else if (questionLower.includes('location') || questionLower.includes('where') || questionLower.includes('located')) {
      subcategory = 'Locations & Directions';
    ...
    } else if (questionLower.includes('cost') || questionLower.includes('charge') || questionLower.includes('free')) {
      subcategory = 'Costs & Charges';
    } else if (questionLower.includes('aircraft') || questionLower.includes('airplane')) {
      subcategory = 'Aircraft Medical Services';
    } else if (questionLower.includes('airside') || questionLower.includes('landside')) {
      subcategory = 'Terminal Areas';
    }
    
    // Generate keywords for better search
    const keywords = [
      'medical', 'health', 'clinic', 'pharmacy', 'first aid', 'emergency',
    ...
    const uniqueKeywords = [...new Set(keywords)].filter(word => 
      !['this', 'that', 'with', 'from', 'they', 'have', 'will', 'your', 'must', 'should', 'there', 'where'].includes(word)
    ...
    questions.push({
      ...
      priority: 1,
      sourceUrl: 'https://www.muscatairport.co.om/en/content/medical-services',
      dataSource: 'manual'
    });
  }
...
async function trainMedicalServicesFAQ() {
  try {
    console.log('🎯 Starting Medical Services FAQ Training...\n');
    
    // Read the medical services FAQ file
    console.log('📖 Reading medical services FAQ document...');
    const content = await fs.readFile(MEDICAL_FAQ_FILE, 'utf8');
    console.log('✅ File read successfully');
    
    // Parse Q&A pairs
    console.log('🔍 Parsing Q&A pairs from document...');
    const questions = parseMedicalFAQ(content);
    ...
    if (successCount > 0) {
      console.log('\n🤖 The chatbot has been trained with medical services FAQ knowledge!');
      console.log('💡 Try asking questions like:');
      console.log('   - "Where is the medical clinic at the airport?"');
      console.log('   - "Is there a pharmacy at Muscat airport?"');
      console.log('   - "What should I do if I need medical help?"');
      console.log('   - "Are medical treatments free at the airport?"');
    }
...
if (require.main === module) {
  trainMedicalServicesFAQ()
    .then(() => {
      console.log('\n✨ Medical services FAQ training completed successfully!');
      process.exit(0);
    })
    .catch((error) => {
...
module.exports = { trainMedicalServicesFAQ, parseMedicalFAQ }; 